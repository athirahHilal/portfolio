...
const Achievement = forwardRef<HTMLDivElement, AchievementProps>(({ isLoading }, ref) => {
  ...
  return (
    <section
      ref={ref}
      className="w-full min-h-screen bg-black text-white flex flex-col items-center pt-20 sm:pt-28 px-4 sm:px-12 font-sans"
    >
      ...
      <div className="w-full flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 justify-items-center mb-16 max-w-7xl">
          {achievements.map((item, index) => (
            <div
              key={index}
              className="achievement-card relative bg-[#1c1c1c] rounded-xl shadow-md overflow-hidden group border border-[#f2acb2]/30 w-full sm:w-[380px] h-[460px]"
            >
              {item.image && (
